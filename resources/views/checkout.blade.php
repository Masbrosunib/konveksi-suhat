...
<body>
  ...
  <!-- Navigation Bar ends -->
    <div class="container">
        ...
        <form action="{{ route('checkout.process') }}" method="POST" enctype="multipart/form-data" class="m-5">
            @csrf
            <div class="form-group">
                <label for="product_id">Product</label>
                <select name="product_id" id="product_id" class="form-control" required>
                    @foreach($products as $product)
                        <option value="{{ $product->product_id }}">{{ $product->product_name }} - Rp{{ number_format($product->product_price, 2, ',', '.') }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="design_image">Design Image (optional)</label>
                <input type="file" name="design_image" id="design_image" class="form-control-file">
            </div>
            <div class="form-group">
                <label for="payment_id">Payment Method</label>
                <select name="payment_id" id="payment_id" class="form-control" required>
                    @foreach($paymentMethods as $paymentMethod)
                        <option value="{{ $paymentMethod->payment_id }}">{{ $paymentMethod->payment_name }}</option>
                    @endforeach
                </select>
            </div>
            @if($errors->any())
